...
Supported agents: **Claude Code**, **OpenAI Codex**, **GitHub Copilot CLI**, **Cursor Agent**, **Grok**, **AGY**, **Hermes**, **OpenCode** (default).
...
- At least one of these AI coding agent CLIs installed and authenticated:
  ...
  - `hermes` — [Nous Hermes CLI](https://github.com/NousResearch/hermes-agent)
...
```bash
for bin in opencode claude codex copilot cursor-agent grok agy hermes; do
...
| Hermes             | `--agent hermes`        | `hermes`       | `RALPH_HERMES_BINARY`        |
...
```
--agent AGENT            Agent to use (opencode|claude-code|codex|copilot|cursor-agent|grok|agy|hermes)
