...
### Rank 2
- **No CLI off-switch for tasks/rotation** (`--no-tasks` missing; rotation can't be unset via CLI) — FIXED 2026-09-05: `--no-tasks` and `--no-rotation` negation flags implemented (src/parse-args.ts) + resume-override precedence tests (RED→GREEN, resume-override-precedence.test.ts).
...
- rotation-active + --model/--agent override prints misleading notice (rotation entry wins for spawn) — FIXED 2026-09-05: clarifying notice added (ralph.ts override block) "rotation is active — the rotation entry picks the spawned agent/model; overrides apply when rotation ends/is unset (--no-rotation)"; test pins it. TOML template comment updated: reuse_skip_min/max marked DEPRECATED (no effect).
