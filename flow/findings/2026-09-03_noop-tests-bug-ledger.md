...
## "BUG: SIGINT handler uses setImmediate before process.exit — partial iteration risk [MEDIUM]"
_source: tests/bugs-error-handling.test.ts_
...
## "BUG: runRalphLoop catch handler doesn't save history before exit [MEDIUM]"
_source: tests/bugs-error-handling.test.ts_
...
## "BUG: captureFileSnapshot failure produces empty file list, breaking struggle detection [MEDIUM]"
_source: tests/bugs-error-handling.test.ts_
...
## "BUG: clearState uses require('fs') instead of imported unlinkSync [MEDIUM]"
_source: tests/bugs-error-handling.test.ts_
...
## "BUG: 20+ process.exit() calls bypass cleanup handlers [HIGH]"
_source: tests/bugs-error-handling.test.ts_
...
## "BUG: history.iterations array grows unbounded with no cap [HIGH]"
_source: tests/bugs-error-handling.test.ts_
...
## "BUG: appendIterationHistory capturesFileSnapshot spawns O(n) subprocesses [HIGH]"
_source: tests/bugs-error-handling.test.ts_
...
## "BUG: loadHistory returns EMPTY_HISTORY on corrupt JSON — all progress lost [HIGH]"
_source: tests/bugs-error-handling.test.ts_
...
## "BUG: saveState errors silently swallowed via try/catch { /* best-effort */ } [HIGH]"
_source: tests/bugs-error-handling.test.ts_
...
## "extracts basename from backslash path"
...
## "T15: abortPromise -> skip review, immediate stop (design verification)"
_source: tests/review-gate.test.ts_
Was always-green design note: abortPromise check breaks loop before review-gate block.
